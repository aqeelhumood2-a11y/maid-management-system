...
import { updateAreaServer, type AreaPatch } from "@/lib/server/catalogService";
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const body = (await request.json().catch(() => ({}))) as AreaPatch;

  try {
    await updateAreaServer(getAdminDb(), id, body, await getActor());
